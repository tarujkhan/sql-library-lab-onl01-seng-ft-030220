...
def select_series_title_with_most_human_characters
   "SELECT series.title FROM series JOIN books ON books.series_id = series.id JOIN character_books on book_id = books.id JOIN characters on character_id = characters.id WHERE characters.species = 'human' GROUP BY species ORDER BY count(*) DESC limit 1"
